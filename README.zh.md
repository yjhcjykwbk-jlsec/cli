...
失败运行原则上不向 stdout 写入任何内容，但有一个例外：`lark-cli api` 与生成的 API 命令
在流式格式（`ndjson`/`table`/`csv`）下会按到达顺序输出每页，故失败前已写出的行会保留。
**首页**的行为保持不变：JSON 和 `--jq` 格式下业务错误仍把原始响应写入 stdout；传输失败及
流式格式下的任何失败则都不写 stdout。
