...
1. `cd <dir> && npm install && npm run dev` 本地开发预览（dev 命令声明见项目根 `spark.json`）；
2. 需要发布时先 `lark-cli apps +create --name <name>` 创建妙搭应用；
3. 在项目根运行 `lark-cli apps +deploy --app-id <返回的 app_id>` 构建并发布（成功后 app id 写入 spark.json，后续免传；见 [lark-apps-deploy.md](lark-apps-deploy.md)）。
